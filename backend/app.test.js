...
describe("API tests", () => {
  ...
  beforeEach(async () => {
    ...
    await request.post("/v1/users/signup").send(userExample);
    let response = await request.post("/v1/users/login").send(userCredentials);
  ...
  test("Search existing", async () => {
    const response = await request.get(`/v1/search?query=${bookExample.title}`);
    const body = JSON.parse(response.text);
  ...
  test("Search not existing", async () => {
    const response = await request.get(`/v1/search?query=thiselementdoesnotexist`);
    const body = JSON.parse(response.text);
  ...
  test("Get a user", async () => {
    ...
    const response = await request.get(`/v1/users/${user._id}`);
    const body = JSON.parse(response.text);
  ...
  test("Get a user that doesn't exist", async () => {
    const response = await request.get(`/v1/users/5fab1591d9fe8e536c4df412`);
    const body = JSON.parse(response.text);
  ...
  test("Get a user without providing a valid ID value", async () => {
    const response = await request.get(`/v1/users/thisisnotauserid`);
    const body = JSON.parse(response.text);
    expect(body.user).toBe(null || undefined);
  ...
  test("User signup", async () => {
    const response = await request.post(`/v1/users/signup`).send(userExampleBis);
  ...
  test("User signup but mail already exists", async () => {
    const response = await request.post(`/v1/users/signup`).send(userExample);
    const body = JSON.parse(response.text);
    expect(body).toHaveProperty("message");
  ...
  test("Delete a user", async () => {
    ...
    const user = await User.findOne({ email: userExample.email });
    const response = await request.delete(`/v1/users/${user._id}`).set(header);
  ...
  test("Adding a review to a book", async () => {
    ...
    const response = await request
      .patch(`/v1/books/${bookExample._id}/review`)
      .send(data)
      .set(header);
  ...
  test("Adding a review to a content", async () => {
    ...
    const response = await request
      .patch(`/v1/contents/${contentExample._id}/review`)
      .send(data)
      .set(header);
  ...
  test("Insert an element in user follow", async () => {
    ...
    const response = await request
      .patch(`/v1/users/${user._id}/follow`)
      .send(data)
      .set(header);
  ...
  test("Insert an element in invalid user follow", async () => {
    ...
    const response = await request
      .patch(`/v1/users/invaliduserid/follow`)
      .send(data)
      .set(header);
    const body = JSON.parse(response.text);
    expect(body).toHaveProperty("error");
  ...
  test("Search content by university with kind not existing", async () => {
    const response = await request.get(
      `/v1/contents?university=THISKINDDOESNOTEXIST/${encodeURI(
        contentExample.validFor[0].university
      )}`
    );
    // Request body from string to object
  ...
  test("Search content by university with university not existing", async () => {
    const response = await request.get(
      `/v1/contents?university=THISUNIVERSITYDOESNOTEXIST`
    );
    // Request body from string to object
    const body = JSON.parse(response.text);
    const { contents } = body;
  ...
  test("Search contents related to the user", async () => {
    const response = await request.get(
      `/v1/users/${contentExample.creator}/contents`
    );
    // Request body from string to object
  ...
  test("Search contents on a user that does not have related contents", async () => {
    const response = await request.get(
      `/v1/users/000000000000000000000000/contents`
    );
    // Request body from string to object
    const body = await JSON.parse(response.text);
    const { resources } = body;
  ...
  test("Increment view count", async () => {
    ...
    const response = await request.post(
      `/v1/contents/${contentExample._id}/view`
    );
    const contentUpdated = await Content.findOne({ _id: contentExample._id });
  ...
  test("Increment view count for not existing content", async () => {
    const response = await request.post(`/v1/contents/notexisting/view`);
  ...
  test("Adding a comment for not existing content", async () => {
    ...
    const response = await request
      .patch(`/v1/contents/notexisting/review`)
      .send(data)
      .set(header);
    expect(response.body).toHaveProperty("error");
    expect(response.status).toBe(500);
  ...
  test("Adding a review to a non existing book", async () => {
    ...
    const response = await request
      .patch(`/v1/books/notexisting/review/`)
      .send(data)
      .set(header);
    expect(response.body).toHaveProperty("error");
    expect(response.status).toBe(500);
